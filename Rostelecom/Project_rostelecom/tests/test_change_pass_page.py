#Для запуска программы введите в терминале: python -m pytest -v --tb=line tests/test_change_pass_page.py
...
class TestChangePassPage():
    def test_TC_ROST013_default_password_recovery_type(self, browser):
    ...
    @pytest.mark.xfail
    def test_TC_ROST014_phone_field_validation_valid_data(self, browser):
    ...
    def test_TC_ROST015_email_field_validation_valid_data(self, browser):
        change_pass_page = ChangePassPage(browser, url_change_page)
        change_pass_page.open()
        change_pass_page.email_field_validation_valid_data()

    def test_TC_ROST016_go_back_button(self, browser):
        change_pass_page = ChangePassPage(browser, url_change_page)
        change_pass_page.open()
        change_pass_page.go_back_button()

    def test_TC_ROST017_link_fut_to_the_user_agreement_page(self, browser):
        change_pass_page = ChangePassPage(browser, url_change_page)
        change_pass_page.open()
        change_pass_page.link_to_the_user_agreement_page()

    def test_TC_ROST018_password_recovery_with_blank_fields(self, browser):
        change_pass_page = ChangePassPage(browser, url_change_page)
        change_pass_page.open()
        change_pass_page.password_recovery_with_blank_fields()

    def test_TC_ROST019_password_recovery_with_blank_captcha(self, browser):
        change_pass_page = ChangePassPage(browser, url_change_page)
        change_pass_page.open()
        change_pass_page.password_recovery_with_blank_fields()

    def test_TC_ROST020_sql_injection_in_a_text_field(self, browser):
        change_pass_page = ChangePassPage(browser, url_change_page)
        change_pass_page.open()
